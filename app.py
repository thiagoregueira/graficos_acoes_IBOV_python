...
# Ajuste para versões novas do yfinance que retornam MultiIndex: (Price, Ticker)
...
if dados.empty:
    st.error(
        f'Não foram encontrados dados para o ativo {acao} no período de {data_inicio.strftime("%d/%m/%Y")} a {data_fim.strftime("%d/%m/%Y")}.'
    )
    st.stop()
...
ult_cotacao = round(
    dados['Close'].iloc[-1].item(),
    2,  # Garante que é um escalar antes de arredondar
...
primeira_cotacao = round(
    dados['Close'].iloc[0].item(),
    2,  # Garante que é um escalar antes de arredondar
)  # Primeira cotação do período
delta = round(((ult_cotacao - primeira_cotacao) / primeira_cotacao) * 100, 2)  # Delta da cotação
volume_total = dados['Volume'].sum().item()  # Garante que é um escalar
var_menor_cotacao = round(((menor_cotacao - primeira_cotacao) / primeira_cotacao) * 100, 2)
var_maior_cotacao = round(((maior_cotacao - primeira_cotacao) / primeira_cotacao) * 100, 2)

# Colunas de métricas
col4, col5, col6, col7 = st.columns(4)


style_metric_cards(background_color='#46629e', border_left_color='#68ba9f', border_color='#68ba9f')
...
# Container principal para Tabela e Gráfico de Área
with st.container():
    col8, col9 = st.columns([1.5, 3], gap='large')

    # Criando uma cópia para exibição na tabela com datas formatadas
    dados_tabela = dados.copy()
    dados_tabela.index = dados_tabela.index.strftime('%d/%m/%Y')
    ...
    with col8:
        ...
        # Ordenando decrescente (mais recente no topo) e Resetando index
        # Usamos 'dados' original para ordenar corretamente por data (datetime), não por string
        df_table = dados.sort_index(ascending=False).copy()

        # Formatando a data APÓS ordenar
        df_table.index = df_table.index.strftime('%d/%m/%Y')
        df_table = df_table.reset_index()

        # Criando Tabela com Plotly para controle total de estilo
        fig_table = go.Figure(
            data=[
                go.Table(
                    header=dict(
                        values=['<b>Data</b>'] + [f'<b>{col}</b>' for col in df_table.columns if col != 'Date'],
                        fill_color='#46629e',
                        align='center',
                    ...
                    cells=dict(
                        values=[df_table[k] for k in df_table.columns],
                        fill_color=['white', '#f5f5f5']
                        * (len(df_table) // 2 + 1),  # Alternando cores levemente para leitura
                        align='center',
                        font=dict(color='black', size=12),
                        height=30,
                        format=[''] + ['.2f'] * (len(df_table.columns) - 1),  # Formatação decimal
                    ),
                )
            ]
        )
        ...
        fig_table.update_layout(
            ...
            height=400,  # Altura fixa com scroll interno nativo do Plotly
        )

        st.plotly_chart(
            fig_table, use_container_width=True
        )  # Plotly Table se ajusta bem, use_container_width ou width='stretch'
    ...
    with col9:
        ...
        if dados.empty:
            ...
            # Correção de warning: use_container_width=True -> width="stretch" (se compatível, caso contrário rever versão streamlit)
